...
def rbf(x_1, x_2, sigma=1.):
    ...
    return torch.exp(- torch.Tensor(dists).type(torch.float32) /(2 * sigma))
...
def hinge_loss(scores, labels):
    ...
    return torch.clamp(1. - labels * scores, min=0.0).mean()### YOUR CODE HERE
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def linear(x_1, x_2):
        ...
        '''
        return torch.matmul(x_1,x_2.transpose(1,0))### YOUR CODE HERE
